...
void dispense(){
	...
	int motor_power = 100;
	int motor_time = 300;
	dispensor_forward_time(motor_power, 200);
	dispensor_stop();
	dispensor_backward_time(motor_power, 500);
	dispensor_stop();
}
...
int can_dispense_ball_bumper(){
	...
	else if (bumper_r != 1 && line_BR == 0){  // right bumper pressed
		turn_left_time(30, 100);
		// forward_time(80, 100);
		motor_stop();
		return 1;
...
void delivery_retry(){
	...
	turn_left_time(80, 1500);
	backward_time(100, 1000);
}
